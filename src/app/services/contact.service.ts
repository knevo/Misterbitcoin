...
export class ContactService {
  //mock the server
  _contacts = this._getInitialContacts();
  ...
  _getInitialContacts() {
    return contactsList.map(contact => new Contact(contact.name, contact.email, contact.phone))
  }
  ...
  public getContactByEmail(email: string): Observable<Contact> {
    let contact = this._contacts.find(contact => contact.email === email)
    contact = { ...contact };
    console.log(contact)
    return contact.name ? of(contact) : Observable.throw(`Email does not match`)
  }
  public saveContact(contact: Contact): Contact {
    return contact._id ? this._updateContact(contact) : this._addContact(contact)
  }

  private _updateContact(contact: Contact): Contact {
    //mock the server work
  ...
  private _addContact(contact: Contact): Contact {
    //mock the server work
